...
// ── Fetch engagement products ──────────────────────────────────────────────
$catStmt = $pdo->prepare("SELECT * FROM categories WHERE slug = 'engagements' AND status = 'active' LIMIT 1");
$catStmt->execute();
$cat = $catStmt->fetch();
$products = [];
if ($cat) {
...
<!-- Hero -->
<div class="occasion-hero" style="background:linear-gradient(135deg,#FDF8F4 0%,#F5EBF0 100%);">
    <div class="container">
        <nav class="breadcrumb" style="justify-content:center;margin-bottom:24px;">
            ...
            <span class="breadcrumb-sep">/</span>
            <span><?= $isRTL ? 'خطوبات' : 'Engagements' ?></span>
        </nav>
        <div class="occasion-hero-icon"><i class="fas fa-gem"></i></div>
        <h1><?= $isRTL ? 'هدايا خطوبة ناعمة بتفاصيل مخصصة' : 'Delicate Engagement Gifts with Personal Touches' ?></h1>
        <p>
            <?= $isRTL
                ? 'لحظة الخطوبة لا تُكرَّر — هدية عطرية مخصصة بأسماء الاثنين وتاريخ اليوم تجعلها أكثر سحراً وأعمق أثراً.'
                : 'An engagement moment is once in a lifetime — a personalised fragrance gift with both names and the date makes it even more magical and meaningful.' ?>
        </p>
        <a href="#quick-form" class="btn-primary" style="display:inline-flex;">
            <i class="fas fa-paper-plane"></i>
            <?= $isRTL ? 'اطلب هدية الخطوبة الآن' : 'Request Your Engagement Gift' ?>
        </a>
    </div>
</div>
...
<div class="container">
    <div class="grid-2" style="gap:48px;align-items:start;">
        ...
        <div class="animate">
            ...
            <h2 style="margin-bottom:20px;"><?= $isRTL ? 'هدية تحكي قصة الخطوبة' : 'A Gift That Tells Your Engagement Story' ?></h2>
            <?php
            $offers = $isRTL ? [
                ['💎','زجاجات عطر صغيرة أنيقة مناسبة للخطوبة'],
                ['💌','كروت خطوبة مخصصة بأسماء الاثنين'],
                ['🎀','تغليف رومانسي بألوان الروز والذهبي'],
                ['✍️','الأحرف الأولى للعروسين على الزجاجة'],
                ['🗓️','تخصيص بتاريخ الخطوبة'],
                ['🎁','بوكسات هدايا متكاملة بكل ما تحتاجه'],
                ['👥','توزيعات للضيوف بكميات كبيرة'],
            ] : [
                ['💎','Elegant mini perfume bottles for engagements'],
                ['💌','Custom engagement cards with both names'],
                ['🎀','Romantic packaging in rose gold & champagne'],
                ['✍️','Couple\'s initials engraved on the bottle'],
                ['🗓️','Personalised with the engagement date'],
                ['🎁','Complete gift boxes with everything included'],
                ['👥','Guest favours in bulk quantities'],
            ];
        ...
        <div class="animate" id="quick-form">
            <div class="quick-form-card">
                ...
                <form method="GET" action="<?= SITE_URL ?>/pages/custom-order.php" style="display:flex;flex-direction:column;gap:12px;">
                    <input type="hidden" name="event_type" value="engagement">
...
<div class="container">
    <div class="section-title animate">
        ...
        <h2><?= $isRTL ? 'هدايا وتوزيعات الخطوبة' : 'Engagement Gifts & Favours' ?></h2>
    </div>
    <div class="grid-4">
...
<div class="container">
    <div class="section-title animate">
        ...
        <h2><?= $isRTL ? 'أفكار لهدايا الخطوبة' : 'Engagement Gift Ideas' ?></h2>
    </div>
    <?php
    $gallery = $isRTL ? [
        ['fa-gem',               'توزيعات عطر أنيقة'],
        ['fa-ribbon',            'بوكسات روز جولد'],
        ['fa-heart',             'زجاجات بأحرف الاثنين'],
        ['fa-envelope-open-text','كروت خطوبة مخصصة'],
        ['fa-gift',              'باكدجات خطوبة متكاملة'],
        ['fa-star',              'توزيعات ورد وعطر'],
    ] : [
        ['fa-gem',               'Elegant Engagement Favours'],
        ['fa-ribbon',            'Rose Gold Boxes'],
        ['fa-heart',             'Couple Initial Bottles'],
        ['fa-envelope-open-text','Custom Engagement Cards'],
        ['fa-gift',              'Complete Engagement Packages'],
...
<div class="container">
    <div class="animate">
        ...
        <h2 style="color:white;margin-bottom:14px;">
            <?= $isRTL ? 'اجعلوا لحظة الخطوبة لا تُنسى' : 'Make Your Engagement Moment Unforgettable' ?>
        </h2>
        <p style="color:rgba(232,213,176,.8);margin-bottom:32px;max-width:500px;margin-inline:auto;">
            <?= $isRTL
                ? 'تواصلوا معنا ونصمم لكم هدية الخطوبة المثالية بكل تفاصيلها.'
                : 'Contact us and we\'ll design your perfect engagement gift down to every last detail.' ?>
        </p>
        <div style="display:flex;gap:14px;justify-content:center;flex-wrap:wrap;">
            <a href="<?= SITE_URL ?>/pages/custom-order.php?event_type=engagement" class="btn-primary">
